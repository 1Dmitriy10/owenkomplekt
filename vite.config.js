...
const rollupInputs = {
  ...
  Корзина_оформление_заказа: path.resolve(__dirname, 'src/html/basket-order.html'),
...
// ИСПРАВЛЕННЫЙ И УПРОЩЕННЫЙ плагин для обработки изображений
const pictureWebpPlugin = () => {
  ...
  return {
    ...
    transformIndexHtml(html) {
      ...
      // Вспомогательная функция для проверки, находится ли изображение внутри picture
      const isInsidePicture = (html, imgIndex) => {
        const substringBefore = html.substring(0, imgIndex);
        const lastPictureOpen = substringBefore.lastIndexOf('<picture');
        const lastPictureClose = substringBefore.lastIndexOf('</picture>');
        
        // Если есть открывающий picture без закрывающего перед изображением
        return lastPictureOpen > lastPictureClose;
      };

      // Функция для генерации правильного пути WebP
      const generateWebpPath = (originalSrc) => {
        // Убираем начальные слеши и точки для единообразия
        let cleanSrc = originalSrc.replace(/^\.?\//, '');
        
        // Если путь начинается с images/, добавляем webp/
        if (cleanSrc.startsWith('images/')) {
          return cleanSrc.replace('images/', 'images/webp/').replace(/\.(png|jpe?g)$/i, '.webp');
        }
      ...
      while ((match = imgRegex.exec(html)) !== null) {
        ...
        if (fullMatch.includes('data-skip-webp') || 
            fullMatch.includes('data-no-webp') ||
            isInsidePicture(html, imgIndex)) {
          processedImages.add(fullMatch);
          continue;
        }
        
        // Генерируем WebP путь
        let webpSrc = generateWebpPath(src);
        
        // Создаем замену с правильной структурой
        const replacement = `<picture><source srcset="${webpSrc}" type="image/webp">${fullMatch}</picture>`;
        
        result = result.replace(fullMatch, replacement);
        processedImages.add(fullMatch);
        
        console.log(`🖼️ Wrapped image: ${src} -> ${webpSrc}`);
      }
      
      console.log(`✅ Processed ${processedImages.size} images`);
      return result;
    }
  };
};
