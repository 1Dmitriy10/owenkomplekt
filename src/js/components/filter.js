...
<div class="filter-container">
    ...
    <div class="range-inputs">
        <input type="number" id="minValue" placeholder="От 3" value="3" min="3" max="170">
        <span>—</span>
        <input type="number" id="maxValue" placeholder="До 170" value="170" min="3" max="170">
    </div>
...
*/
  ...
  function updateHandlePosition(percent) {
    ...
    if (currentHandle === 'min') {
      if (value >= maxVal) {
        value = maxVal - (rangeMax - rangeMin) * 0.01; // Сдвигаем чуть назад, чтобы не было равенства
  ...
  function stopDrag() {
    if (isDragging) {
      isDragging = false;
      currentHandle = null;
      document.querySelectorAll('.slider-handle').forEach(h => h.classList.remove('active'));
    }
  }
